...
また、レンダラーおよび各エフェクトは、基本的にdeterministicであることが求められます。そうでないと、時間範囲を指定して動画としてレンダリングを行ったときなどに、レンダリングするたびに結果が変わってしまうことになり得ます。

座標系は、左下が[-1, -1]、右上が[+1, +1]です。中央が[0, 0]です。

ノードごとに異なる解像度や比率でテクスチャを出力する可能性があります。ただ、通常入力テクスチャのサンプリング処理は、動的に生成される専用関数をシェーダー内で使う形になり、その場合は入力テクスチャの解像度・アスペクト比についてシェーダー側は意識する必要のない仕組みになっています。

ノード間の接続は、deterministicです。つまり、expressionで動的に配線を変えるといった挙動はサポートしません。

ただし、複数の入力を受け取り、そのうちのどれか一つを選んで出力するようなエフェクトのパラメータにexpressionを使用することで、事実上接続が動的に変わっているような効果は再現できます。
