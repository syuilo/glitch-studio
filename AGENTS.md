...
各レイヤーの `compositing` はモジュールの `paramValues` と独立したパラメータ値です。定義・既定値は `packages/shared/src/timeline-compositing.ts` にあり、expression・環境変数・inline/参照automation graphはモジュールパラメータと同じ評価器を使用します。時刻はレイヤー内の相対時刻、参照グラフは使用するVisual Moduleのグラフです。

- 初期値は通常合成、不透明度1、無変形です。通常合成ではモジュールは透明背景の素材を出力し、タイムライン側で背景に重ねます。どの合成方法でもInから背景を参照できます。
- `replace` は背景を含めた完成結果を出力するモジュール向けです。変形後の出力で透明部分も含めて置き換えます。opacityは背景からその出力へのRGBAの補間量で、0なら背景、1なら出力そのものです。
- transformはモジュールの出力全体に適用します。背景を加工する処理の中心・作用範囲だけを動かす機能ではありません。中央基準で拡縮→回転→移動し、範囲外は透明です。位置の1は画面幅/高さの半分、回転の1は時計回り180度です。位置と拡縮はX/Y別のscalarで、負の倍率は反転、0は透明化として扱います。
