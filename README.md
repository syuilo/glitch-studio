...
### 特徴
* 画像にカスタマイズ可能なジッター、歪み、ノイズといった効果を適用してオリジナルのグリッチエフェクトを作成できます。
* デジタルなグリッチだけではなく、アイデア次第でCRTやVHSのようなアナログなグリッチも作成できます。
* エフェクトにはランダムな効果を与えられるものもありますが、乱数のシードもパラメータとして与えられるので、実行するたびに結果が変わってしまう心配はありません。
...
Glitch Studioでは式評価エンジンに[Math.js](https://mathjs.org/)を使用しているため、詳しい情報については[ドキュメント](https://mathjs.org/docs/reference/functions.html)をご覧ください(`math.`のプレフィックスは不要)。
...
### マスク
特定の範囲にだけエフェクトを適用したい場合は、マスクを設定します。
「Mask」パラメータにマスク画像を指定することで、マスク画像に応じて透明度が決定されます。
マスク画像のあるピクセルが白いほど透明度は下がり、黒いほど透明になります。
