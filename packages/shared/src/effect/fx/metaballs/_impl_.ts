...
export default implementEffect<typeof definition>({
	...
	init: ({ wgpu }) => {
		...
		const pipelines = createShaderInputPipeline({
			...
			schema: { background: 'color', colors: { array: 'color' } },
		...
		return {
			render: ctx => {
				...
				uniformValues.set({
					...
					count: Math.max(1, Math.min(20, ctx.params.count)),
					size: Math.max(0, Math.min(1, ctx.params.size)),
				});
				device.queue.writeBuffer(uniformBuffer, 0, uniformValues.arrayBuffer);
				const variant = pipelines.update({ background: ctx.params.background, colors: ctx.params.colors }, output);
