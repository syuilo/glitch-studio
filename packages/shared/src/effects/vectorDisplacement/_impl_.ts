...
export default implementEffect<typeof definition>({
	...
	init: ({ wgpu, resolution }) => {
		...
		const uniformValues = makeStructuredView(makeShaderDataDefinitions(code).uniforms.params);
		...
		return {
			render: ctx => {
				uniformValues.set({
					amount: ctx.params.amount, rotation: -(ctx.params.rotation ?? 0) * Math.PI,
					flipX: ctx.params.flipX ? -1 : 1, flipY: ctx.params.flipY ? -1 : 1,
