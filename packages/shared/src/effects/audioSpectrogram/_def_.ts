...
export default defineEffect({
	...
	paramDefs: {
		...
		smoothing: { dataType: 'scalar', ui: { control: 'range', min: 0, max: 2, step: 0.01 }, label: 'Smoothing (seconds)', default: () => ({ inputSource: 'literal', value: 0 }) },
		minFrequency: { dataType: 'scalar', ui: { control: 'number', min: 0, max: 96000, step: 1 }, label: 'Minimum frequency (Hz)', default: () => ({ inputSource: 'literal', value: 20 }) },
		maxFrequency: { dataType: 'scalar', ui: { control: 'number', min: 1, max: 96000, step: 1 }, label: 'Maximum frequency (Hz)', default: () => ({ inputSource: 'literal', value: 20000 }) },
		logarithmic: { dataType: 'bool', ui: { control: 'bool' }, label: 'Logarithmic frequency', default: () => ({ inputSource: 'literal', value: false }) },
		minDb: { dataType: 'scalar', ui: { control: 'range', min: -120, max: -1, step: 1 }, label: 'Minimum dB', default: () => ({ inputSource: 'literal', value: -80 }) },
		maxDb: { dataType: 'scalar', ui: { control: 'range', min: -60, max: 20, step: 1 }, label: 'Maximum dB', default: () => ({ inputSource: 'literal', value: 0 }) },
		duration: { dataType: 'scalar', ui: { control: 'range', min: 0.5, max: 60, step: 0.5 }, label: 'Time span (seconds)', default: () => ({ inputSource: 'literal', value: 10 }) },
